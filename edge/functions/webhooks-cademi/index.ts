...
import { validateCademiSignature, generateEventHash } from '../../lib/crypto.ts';
...
const logger = createLogger({ platform: 'cademi' });
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const signature = req.headers.get('x-cademi-signature') || '';
    const secret = Deno.env.get('CADEMI_WEBHOOK_SECRET') || '';
    
    // Validar assinatura
    if (!validateCademiSignature(body, signature, secret)) {
      logger.warn('Assinatura inválida', { signature });
      const client = getSupabaseClient();
      const platformId = await getPlatformId(client, 'cademi');
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-cademi-signature',
};
