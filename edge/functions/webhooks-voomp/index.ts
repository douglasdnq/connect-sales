...
import { validateVoompSignature, generateEventHash } from '../../lib/crypto.ts';
...
const logger = createLogger({ platform: 'voomp' });
...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const signature = req.headers.get('x-voomp-signature') || '';
    const secret = Deno.env.get('VOOMP_WEBHOOK_SECRET') || '';
    
    // Validar assinatura
    if (!validateVoompSignature(body, signature, secret)) {
      logger.warn('Assinatura inválida', { signature });
      const client = getSupabaseClient();
      const platformId = await getPlatformId(client, 'voomp');
    ...
    const eventHash = generateEventHash(body);
    const payload = JSON.parse(body);
    
    logger.info('Webhook recebido', { 
      event_type: payload.sale_status,
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-voomp-signature',
};
