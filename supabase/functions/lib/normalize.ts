  ...
  try {
    ...
    if (data.order_status.toLowerCase() === 'refunded') {
      return {
        ...
        amount: data.Commissions.settlement_amount,
    ...
    if (['paid', 'approved'].includes(data.order_status.toLowerCase())) {
      return {
        type: 'order_paid',
        platform_order_id: data.order_id,
        paid_at: data.approved_date || data.updated_at,
        amount: data.Commissions.settlement_amount,
        fee: data.Commissions.kiwify_fee,
    ...
    return {
      ...
      customer: {
        ...
        phone_e164: data.Customer.mobile,
        cpf: data.Customer.cnpj,
      },
      products: [{
        product: {
          platform_product_id: data.Product.product_id,
          name: data.Product.product_name,
          list_price: data.Commissions.product_base_price,
        },
        qty: 1,
        unit_price: data.Commissions.product_base_price,
      }],
      order_date: data.created_at,
      gross_amount: data.Commissions.product_base_price,
      net_amount: data.Commissions.settlement_amount,
